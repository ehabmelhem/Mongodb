...
const secondsecham = new mongoose.Schema({
  age: Number,
});
const firstCollection = mongoose.model("firstCollection", firstsechma);
const secondCollection = mongoose.model("secondCollection", secondsecham);
...
app.get("/add-age", (req, res) => {
  const { age } = req.query;
  var add = new secondCollection({ age });
  add.save().then(() => {
    console.log("add " + age + " to the db");
  });
  res.send({ age });
});
